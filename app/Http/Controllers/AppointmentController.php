...
//        dd($data, $patient);
    ...
    {
        if ($request->ajax()) {
            return DataTables::of((new DoctorAppointmentDataTable())->get($request->only(['status', 'filter_date', 'payment_type',])))->make(true);
        }
        $appointmentStatus = Appointment::ALL_STATUS;
        $paymentStatus = getAllPaymentStatus();
    ...
    {
        $checkRecord = User::whereEmail($request->email)->whereType(User::DOCTOR)->first();
