...
class AppointmentRepository extends BaseRepository
{
    ...
    /**
     * @var GoogleCalendarController $googleCalendarController
     */
...
//            $visit = Visit::create($input);
    ...
    {
        $data['doctors'] = Doctor::with('user')->get()->where('user.status', User::ACTIVE)->pluck('user.full_name', 'id');
    ...
    {
        $data = Patient::with('user')->get()->where('user.status', User::ACTIVE)->where('patient_unique_id', $patient)->first();
//        $data['doctors'] = Doctor::with('user')->get()->where('user.status', User::ACTIVE)->pluck('user.full_name', 'id');
//        $data['patientStatus'] = Appointment::PATIENT_STATUS;
//        dd($patient);
        return $data;
    ...
    {
        $input = Appointment::with(['patient.user', 'patient.address', 'doctor.user', 'services'])->where('id', $input->id)->first();
    ...
    public function getAppointmentsData()
    {
        ...
        foreach ($appointments as $key => $appointment) {
            ...
            $start = Carbon::createFromFormat('d-m-Y h:i A', $appointment->date.' '.$startTime);
            $end = Carbon::createFromFormat('d-m-Y h:i A', $appointment->date.' '.$endTime);
            ...
            'line_items'           => [
                [
                    'price_data'  => [
                        ...
                        'unit_amount'  => $input['payable_amount'] * 100,
